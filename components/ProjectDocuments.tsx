...
export function ProjectDocuments({ projectId, readOnly = false }: Props) {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    if (atLimit) { setError(`Limit reached: at most ${MAX_DOCS} documents per project.`); return; }
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        {!readOnly && <span className="text-xs text-gray-400">{docs.length} / {MAX_DOCS}</span>}
      </div>
        ...
        <>
          ...
          {docs.length === 0 && (
            <p className="text-xs text-gray-400 mb-3">
              {readOnly ? 'No documents.' : 'No documents uploaded yet.'}
            </p>
          )}
