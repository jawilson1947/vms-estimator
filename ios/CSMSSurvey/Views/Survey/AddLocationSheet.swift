...
struct AddLocationSheet: View {
    ...
    var body: some View {
        ...
        .fullScreenCover(isPresented: $showInterview) {
            VoiceInterviewView(manager: interview) { showInterview = false }
                .onDisappear {
                    interview.stop()
                    voice.setEnabled(true)   // revive the always-on command listener
                }
        }
    ...
    private func startVoiceInterview() {
        ...
        // AVAudioEngine owns the mic. Two recognizers on bus 0 is what made
        // nothing get recognized. Revived on the cover's .onDisappear.
        voice.setEnabled(false)
        showInterview = true
        interview.start(
            area: areaName, floor: floor, notes: surveyNotes,
            onSave: {
                // "Save": commit and close. Manager already speaks "Saved."
            ...
            onSaveAndNext: {
                // "Save and Next": commit, manager resets and keeps listening.
                areaName    = interview.areaName
                floor       = interview.floor
                surveyNotes = interview.surveyNotes
